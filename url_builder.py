...
def get_search_url(url, jql):
    encoded = urlencode(dict(jql=jql))
...
def main(wf):
    ...
    if missing_configs:
        notify("Missing Configuration", "Run the `jirasetup` keyword to set config.")
        raise Exception(
            "Missing configs: {}. Run `jirasetup` to configure".format(missing_configs)
        )
    ...
    result = None
    if args["url_type"] == "issue":
        result = get_issue_url(wf.settings[JIRA_URL], args["issue_key"])
    elif args["url_type"] == "search":
        result = get_search_url(wf.settings[JIRA_URL], JQL_QUERY)
    elif args["url_type"] == "board":
